...
export const useStateFilms = () => {
  ...
  if (!films) {
    throw Error("useStateFilms must ve call within FilmsContextProvider");
  }
  return films;
};
...
export const useDispatchFilms = () => {
  ...
  if (!setFilms) {
    throw Error("useDispatchFilms must ve call within FilmsContextProvider");
  }
  return setFilms;
};
...
export const useLoadFilms = () => {
  ...
  const loadFilms = useCallback(() => {
    return api.films.fetchAll()
              .then((films) => setFilms(sortFilms(films)));
  }, [setFilms]);
  return loadFilms;
};
